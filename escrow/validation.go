...
func getSignerOfAuthTokenForFreeCall(payment *FreeCallPayment) (signer *common.Address, err error) {
	//signer-token = (user@mail, user-public-key, token_issue_date), this is generated by Market place Dapp
	signer, err = getUserAddressForTokenChecks(payment)
	message := bytes.Join([][]byte{
		[]byte(payment.UserId),
		[]byte(signer.Hex()),
		bigIntToBytes(payment.AuthTokenIssueDateBlockNumber),
	}, nil)
...
func getUserAddressForTokenChecks(payment *FreeCallPayment) (signer *common.Address, err error) {

	log.WithField("block number:",payment.CurrentBlockNumber)
	message := bytes.Join([][]byte{
		[]byte(FreeCallPrefixSignature),
		[]byte(payment.UserId),
		[]byte(config.GetString(config.OrganizationId)),
